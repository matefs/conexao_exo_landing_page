...
const experienceLink = name => `https://wa.me/5548991109531?text=${encodeURIComponent(`Olá! Vim pelo site da Conexão EXO e quero saber mais sobre ${name}.`)}`

function App() {
  useEffect(() => {
    Object.entries(cssVariables).forEach(([key, value]) => document.documentElement.style.setProperty(key, value))
    const lenis = new Lenis({ autoRaf: true, lerp: 0.075, smoothWheel: true, wheelMultiplier: 0.85, anchors: { offset: -76, duration: 1.2 } })
    window.lenis = lenis
    return () => {
      lenis.destroy()
      delete window.lenis
